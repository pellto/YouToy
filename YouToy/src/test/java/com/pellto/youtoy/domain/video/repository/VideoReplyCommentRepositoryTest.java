...
@DataJpaTest
class VideoReplyCommentRepositoryTest {
  ...
  void saveSuccessTest() {
    ...
    var savedReply = videoReplyCommentRepository.save(reply);
  ...
  void findAllByParentSuccessTest() {
    var parentComment = beforeSetting();
    var reply = VideoReplyFactory.createBeforeSaved(parentComment);
    videoReplyCommentRepository.save(reply);

    var foundReplies = videoReplyCommentRepository.findAllByParentComment(parentComment);

    Assertions.assertThat(foundReplies).isNotEmpty();
    Assertions.assertThat(foundReplies.size()).isEqualTo(1);
    Assertions.assertThat(foundReplies.get(0).getClass()).isEqualTo(VideoReplyComment.class);
  ...
  void findByIdSuccessTest() {
    ...
    var nullableReply = videoReplyCommentRepository.findById(reply.getId());
  ...
  void deleteByIdSuccessTest() {
    ...
    videoReplyCommentRepository.deleteById(replyId);

    var deletedReply = videoReplyCommentRepository.findById(replyId);
  ...
  void deleteWithParentCommentSuccessTest() {
    ...
    var deletedReplies = videoReplyCommentRepository.findAll();
